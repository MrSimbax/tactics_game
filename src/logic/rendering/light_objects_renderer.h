...
{
public:
    explicit light_objects_renderer(std::vector<std::vector<std::shared_ptr<simple_color_object>>> light_objects);

    void render(shader_program& program, int layer) const;

private:
    std::vector<std::vector<std::shared_ptr<simple_color_object>>> light_objects_{};
